...
with app.app_context():
    ...
    # Xóa toàn bộ dữ liệu cũ để tránh lỗi trùng lặp
    db.drop_all()
    db.create_all()
    ...
    # 4. TẠO ROUTES (Tuyến đường)
    # Cột: origin_station_id, destination_station_id, distance_km, estimated_duration_minutes
    # Lưu ý: Ở đây dùng 'distance_km' và 'estimated_duration_minutes' như trong models.py của bạn
    ...
    # Cột: route_id, vehicle_id, departure_time, arrival_time, base_price, status
    
    # Chuyến 1: HN -> ĐN, đi bằng xe Limousine (v1), giá cao
    # Đi vào 8h sáng ngày mai
    depart_1 = datetime.now() + timedelta(days=1, hours=8)
    # Đến nơi sau 14 tiếng (theo route 1)
    ...
    # Chuyến 2: HCM -> ĐL, đi bằng xe Giường nằm (v2), giá rẻ
    # Đi vào 10h tối ngày kia
